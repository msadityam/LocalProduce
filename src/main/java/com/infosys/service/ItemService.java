...
public class ItemService {
	...
	public ResponseEntity<String> purchaseItem(Long userId, String itemName, int quantity) {
		...
		if (user.getCoins() < totalCost) {
			String errorMessage = "Insufficient coins to make the purchase '" + itemName
					+ "' in user's wallet. required coins: '" + totalCost + "' user has: '" + user.getCoins();
	...
	public void addItemToInventory(ItemRequest itemRequest) {
		...
		// Save the updated inventory item (this will either update existing or save
		// new)
	...
	public void addItemToUserWallet(AddItemToWalletRequest itemRequest) {
		...
		Item item = itemRepository.findByName(itemName).orElseGet(() -> {
	...
	public void deleteItemFromInventoryByName(String name) {
		// Retrieve the item from the database
		Item item = itemRepository.findByName(name).orElseThrow(() -> new RuntimeException("Item not found"));
